...
import { updatePost } from '@/app/(login)/actions';
import { useRouter, useParams } from 'next/navigation';
import { PostWithDetails, User } from '@/lib/db/schema';
  ...
  return (
    <div className="space-y-2">
      ...
      <Textarea
        ...
        className={`transition-all ${isEditorFocused ? 'ring-2 ring-ring' : ''}`}
      />
      {value && (
...
const EditPostPage = () => {
  ...
  const { data: post, error: postError, isLoading: postLoading } = useSWR<PostWithDetails>(
  ...
  const form = useForm<PostFormData>({
    ...
    values: post ? {
      ...
      visibility: post.visibility as "public" | "friends_only" | "private",
  ...
  const onSubmit = async (data: PostFormData) => {
    ...
    try {
      ...
      const result = await updatePost({ error: '' }, formData);
